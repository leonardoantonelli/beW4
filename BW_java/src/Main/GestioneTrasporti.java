...
Distributore d3 = new Distributore();
		...
		while(true) {
			System.out.println("Seleziona una delle seguenti opzioni:"
					...
					+ "\n 3 GESTIONE BIGLIETTI/ABBONAMENTI " //Visualizza tutti i biglietti/abbonamenti emessi in un range di date 
					+ "\n 4 GESTIONE TESSERE " //Gestione stato distributori
					+ "\n 5 GESTIONE DISTRIBUTORI " //Verifica validità della tessera inserendo il numero e stampa tutte le tessere
			...
			case 1:	
				...
				while(!esci) {
					...
					System.out.println(">> Seleziona un delle due opzioni di acquisto 1 = Distributore automatico 2 = Rivenditore | 0 per uscire.");
					...
					case 1:
						System.out.println(">> Seleziona uno dei distributori Distributore = 1 Distributore = 2 Distributore = 3");
					...
					case 2:
						opzionesalvata = 2;
						break;
						default: System.out.println("Prova uno valore valido");
					}
				
					//Verifica il risultato di opzione salvata
					
					if(opzionesalvata == 1 || opzionesalvata == 2 ) {
						System.out.println(">> Seleziona Titolo di viaggio da aggiungere 1 = Abbonamento 2 = Biglietto | 0 per uscire.");
					...
					while(!esci) {
						...
						if(agg == 0) {
							...
							if(TesseraDAO.isTessereEmpty() == 0) {
								System.out.println("Per gli abbonamenti è necessaria una tessera! Prima creane una!");
							} else {
			...
			case 3:
				...
				case 1:
					...
					System.out.println("In totale sono stati emessi : " + lista.size() + " Biglietti/Abbonamenti \n");
					break;
				case 2:
					...
					Query q1 = em.createQuery("SELECT t FROM TitoloDiVIaggio t WHERE t.dataemissione >= :d AND t.dataemissione <= :d1"); 
					q1.setParameter("d", data);
					q1.setParameter("d1", data2);
					List<TitoloDiVIaggio> lista1 = q1.getResultList();
					System.out.println("In totale sono stati emessi : " + lista1.size() + " Biglietti/Abbonamenti \n");
					break;
				case 3:
					...
					try {
						 ...
						 case 1:
							 ...
							 Query q2 = em.createQuery("SELECT t FROM TitoloDiVIaggio t WHERE t.puntoemissione = :riv");
							 q2.setParameter("riv", riv);
							 List<TitoloDiVIaggio> lista2 = q2.getResultList();
							 System.out.println("In totale sono stati emessi : " + lista2.size() + " Biglietti/Abbonamenti da Rivenditori Autorizzati \n");
							 break;
						 case 2:
							 Ticket_Office dis = Ticket_Office.DISTRIBUTORE_AUTOMATICO;
							 Query q3 = em.createQuery("SELECT t FROM TitoloDiVIaggio t WHERE t.puntoemissione = :dis");
							 q3.setParameter("dis", dis);
							 List<TitoloDiVIaggio> lista3 = q3.getResultList();
							 System.out.println("In totale sono stati emessi : " + lista3.size() + " Biglietti/Abbonamenti da Distributori Autorizzati \n");
							 break;
						 }
					} catch (NumberFormatException e) {
						System.out.println("Input Errato! Utilizza un numero");
			...
			case 5:
				Boolean back = false;
					while(!back) {
						System.out.println(">> Seleziona il distibutore automatico da modificare: \n Distributore 1 - Distributore 2 - Distributore 3 | 0 per uscire");
						int opzione = s.nextInt();
						s.nextLine();
						switch(opzione) {
						case 0:
